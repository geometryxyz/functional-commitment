...
pub(crate) fn arithmetize_individual_matrix<F: PrimeField>(
    ...
    m_prefix: &str,
) -> IndividualMatrixArithmetization<F> {
    ...
    for (row_index, row) in m.into_iter().enumerate() {
        for (val, col_index) in row {
            ...
            let col_val = elems[output_domain.reindex_by_subdomain(input_domain, *col_index)];
...
pub fn fc_arith<F: PrimeField>(
    ...
    let val_to_append_row = if is_diag_matrix {
        F::one()
    } else {
        last_row
    };
...
pub fn fc_arith_diag<F: PrimeField>(
    m: &Matrix<F>,
    interpolation_domain: GeneralEvaluationDomain<F>,
    output_domain: GeneralEvaluationDomain<F>,
    m_prefix: &str,
    ...
    // now append until n-1 with w^t+i, w*
